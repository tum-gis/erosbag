...
pub fn repair_rosbag(rosbag_directory_path: impl AsRef<Path>) {
    info!("Rosbag: {}", rosbag_directory_path.as_ref().display());

    normalize_rosbag_files(rosbag_directory_path.as_ref());
...
fn normalize_rosbag_files(rosbag_directory_path: impl AsRef<Path>) {
    let rosbag_directory_name = rosbag_directory_path
        .as_ref()
        .file_name()
        .unwrap()
        .to_str()
    ...
    let main_path = rosbag_directory_path.as_ref().join(direct_filename);

    if main_path.exists() {
        let indexed_filename = PathBuf::from(rosbag_directory_name.to_string() + "_0")
            .with_extension(erosbag_core::SQLITE3_EXTENSION);
        let indexed_path = rosbag_directory_path.as_ref().join(indexed_filename);
